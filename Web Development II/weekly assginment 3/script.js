...
function showData() {
	const tableData = photos.map((photo) => {
			return `<tr><td class="id">${photo.id}</td><td class="albumId">${photo.albumId}</td><td class="title">${photo.title}</td><td class="image"><img src="${photo.url}"></td><td class="thumbnail"><img src="${photo.thumbnailUrl}"></td></tr>`;
		}).join('');
